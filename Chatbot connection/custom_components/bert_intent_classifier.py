...
class BertIntentClassifier(GraphComponent, IntentClassifier):
    ...
    def __init__(
        ...
        if not os.path.exists(model_path):
            logging.error(f"❌ BERT model path not found: {model_path}")
            raise RuntimeError(f"BERT model not found at {model_path}. Ensure the model is placed correctly.")
        ...
        self.intent_mapping = {
            0: "affirm", 1: "agree", 2: "bird_info_generate", 3: "bot_challenge", 4: "deny",
            5: "feedback_negative", 6: "feedback_positive", 7: "general_question", 8: "goodbye",
            9: "greet_checking_in", 10: "greet_good_afternoon", 11: "greet_good_evening",
            12: "greet_good_morning", 13: "greet_good_night", 14: "greet_hi", 15: "help",
            16: "image_classification", 17: "keyword_finder", 18: "mood_excited", 19: "mood_great",
            20: "mood_unhappy", 21: "non_birds", 22: "range_prediction", 23: "repeat_request",
            24: "thank_you",
        }
    ...
    def create(
        ...
        logging.warning(f"⚠️ Model path {model_path} not found. Ensure the model exists.")
        return cls(config, model_storage, resource, execution_context)
    ...
    def process(self, messages: List[Message]) -> List[Message]:
        ...
        inputs = {key: val.to(self.device) for key, val in inputs.items()}  # Move inputs to the same device
        ...
        softmax_values = torch.nn.functional.softmax(logits, dim=1)  # Corrected softmax usage

        for i, message in enumerate(messages):
            predicted_class = torch.argmax(logits[i]).item()
            confidence = float(softmax_values[i, predicted_class])

            if confidence < 0.3:
                intent_name = "fallback"
                logging.warning(f"⚠️ Low confidence ({confidence:.2f}). Using fallback intent.")
            else:
    ...
    def persist(self, file_name: str, model_dir: str) -> None:
        try:
            ...
            logging.info(f"✅ Model persisted successfully to {save_path}.")
        except Exception as e:
